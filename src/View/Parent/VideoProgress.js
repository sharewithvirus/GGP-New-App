...
import { getAllInProgressVideoByKidId } from '../../api/UserPlaylistVideo';
...
export default function VideoProgress() {
    ...
    const getAllCompleted = async () => {
        ...
        try {
            ...
            let { data: res } = await getAllInProgressVideoByKidId(currentKid._id)
            if (res.data) {
    ...
    const renderItem = ({ item, index }) => {
        return (
            <View style={[styles.imgView, index == data.length - 1 && { marginBottom: Spacing.MARGIN_20 }]}>
                <TouchableOpacity style={{ flex: 1, backgroundColor: "transparent", zIndex: 150, position: "relative" }}>
                    <View style={{
                ...
                <View style={{ flexDirection: 'column', backgroundColor: "transparent", marginLeft: Spacing.MARGIN_15, flex: 1 }}>
